        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reasons.map((reason, index) => {
            ...
            return (
              ...
              >
                <Card className="shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full border-t-4 border-accent-500 bg-white group">
                  <CardBody className="p-8 flex flex-col items-center text-center gap-4">
                    {/* Icon with Background */}
                    <div className="w-20 h-20 rounded-2xl bg-accent-500 flex items-center justify-center shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all duration-300">
                      <Icon className="w-10 h-10 text-black" />
                    </div>

                    {/* Title */}
                    <h3 className="text-xl md:text-2xl font-bold text-gray-900 group-hover:text-accent-600 transition-colors duration-300">
                      {t(reason.titleKey)}
                    </h3>
